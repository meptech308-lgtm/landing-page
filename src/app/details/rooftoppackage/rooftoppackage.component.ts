...
export class RooftoppackageComponent {
  ...
  ngOnInit() {
    this.title.setTitle(
      'Daikin Rooftop Units UAE | Commercial HVAC Systems | MEPTECH',
    );

    this.meta.updateTag({
      name: 'description',
      content:
        'Get Daikin rooftop package units in the UAE for powerful and efficient commercial cooling. Ideal for large spaces, with easy installation and reliable HVAC performance.',
    });
    this.meta.updateTag({
      name: 'keywords',
      content:
        'Daikin rooftop units UAE, rooftop AC UAE, commercial HVAC UAE, rooftop package unit Dubai',
    });
  }
}
